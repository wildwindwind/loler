...
<div class="swap">	      	
	<table class="list-box">
		<form id="pForm" method="post" action="${base}/admin/role_list.htm">	
...
<script type="text/javascript">
	...
	function role_add(){
		Pop.window("${base}/admin/role_add.htm", 500, 300, "新增角色", 3000);
	}
	
	function role_edit(id){
		Pop.window("${base}/admin/role_edit.htm?id="+id, 500, 300, "修改角色", 3000);
	}
	
	function role_delete(id){
	   	Pop.confirm("确定要删除这些记录吗?", function() {
			var url = "${base}/admin/role_delete.htm";
			$.ajax( {
				type : 'GET',
				dataType : 'text',
				async : false,
